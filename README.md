...
> **Nota de diseño:** las capas Bronze/Silver/Gold están implementadas como datasets de BigQuery, no como carpetas en Cloud Storage. Ambos approaches son válidos en la industria — la elección depende del volumen y la estructura del dato, y de qué motor se usa para transformar. Un data lake en Cloud Storage (Parquet/Avro particionado, procesado con Dataflow/Spark) tiene sentido para volúmenes grandes o datos semi/no estructurados que necesitan ser leídos por múltiples engines. Acá el dato es pequeño y estructurado (cotizaciones de un JSON de API), y la transformación es 100% SQL/pandas — no hay necesidad de un motor de procesamiento distribuido. Implementar las capas directamente en BigQuery simplifica el pipeline al no depender de infraestructura de storage externa, y es el patrón que usan herramientas como dbt (que materializa modelos como tablas dentro del warehouse). Es también el approach dominante en el "modern data stack" actual (EL/ELT + warehouse + dbt).
